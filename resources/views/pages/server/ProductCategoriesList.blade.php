        ...
        <table class="display" id="basic-1">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $item->name}}</td>
              <td><img class="img-thumbnail" width="100" height="100" src="{{ URL::to('/') }}/server/assets/images/productcategory/{{$item->img}}"></td>
              <td>
                @if($item->status==1)
              ...
              <td class="flex-column align-items-center justify-content-around">
                <a href="{{route('LoaiSanPham.show',$item->id)}}" method="get">
                  <i class="icofont icofont-paper" style="font-size:20px;color:green"></i>
                </a>
                <a href="{{route('LoaiSanPham.edit',$item->id)}}">
                  <i class="icofont icofont-pencil-alt-5" style="font-size:20px;color:blue"></i>
                </a>
                <a href="{{URL::to('/XoaLoaiSanPham',$item->id)}}" onclick="return confirm('Bạn muốn xóa danh mục này ?')">
                  <input type="hidden" name="_token" value="{{csrf_token()}}">
