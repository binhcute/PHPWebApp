      ...
      <table class="table table-hover">
        ...
        @foreach($logo as $item)
          <tr>
            ...
            <td>{{ $item->name}}</td>
            <td><img src="server/assets/images/logo/{{$item->img}}"></td>
            <td>
              @if($item->status==1)
            ...
            <td class="flex-column align-items-center justify-content-around">
              <form action="{{route('Logo.show',$item->id)}}" method="get">
              ...
              <form action="{{route('Logo.edit',$item->id)}}" method="get">
              ...
              @if($item->status == 1)
              <form action="{{URL::to('/Logo/disabled/'.$item->id)}}" method="post">
              ...
              @else
              <form action="{{URL::to('/Logo/enabled/'.$item->id)}}" method="post">
                <input type="hidden" name="_token" value="{{csrf_token()}}">
                <input type="hidden" name="_method" value="put" />
