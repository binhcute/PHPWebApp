      ...
      <table class="table table-hover">
        ...
        @foreach($color as $item)
          <tr>
            ...
            <td>{{ $item->name}}</td>
            <td><img src="server/assets/images/color/{{$item->img}}"></td>
            <td>
              @if($item->status==1)
            ...
            <td class="flex-column align-items-center justify-content-around">
              <form action="{{route('MauSac.show',$item->id)}}" method="get">
              ...
              <form action="{{route('MauSac.edit',$item->id)}}" method="get">
              ...
              @endif
              <form action="{{route('MauSac.destroy',$item->id)}}" method="post">
                <input type="hidden" name="_token" value="{{csrf_token()}}">
