      ...
      <thead>
        ...
        @foreach($article as $item)
          <tr>
            ...
            <td>{{ $item->name}}</td>
            <td><img class="img-thumbnail" width="100" height="100" src="{{ URL::to('/') }}/server/assets/images/article/{{$item->img}}"></td>
            <td>
                @if($item->status==1)
            ...
            <td class="flex-column align-items-center justify-content-around">
                <a href="{{route('BaiViet.show',$item->id)}}" method="get">
                ...
                <a href="{{URL::to('/XoaBaiViet',$item->id)}}" onclick="return confirm('Bạn muốn xóa danh mục này ?')">
                  <input type="hidden" name="_token" value="{{csrf_token()}}">
